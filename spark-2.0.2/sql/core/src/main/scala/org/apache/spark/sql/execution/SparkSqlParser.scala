...
//import org.apache.hadoop.hive.hbase._
...
class SparkSqlAstBuilder(conf: SQLConf) extends AstBuilder {
  ...
  override def visitCreateTable(ctx: CreateTableContext): LogicalPlan = withOrigin(ctx) {
    ...
    val tableDesc = CatalogTable(
      ...
      properties = properties ++ storage.params,
      comment = comment)
  ...
  override def visitCreateFileFormat(
    ...
    (ctx.fileFormat, ctx.storageHandler) match {
      ...
      case (null, c: StorageHandlerContext) =>
        visitStorageHandler(c)
  ...
  override def visitStorageHandler(ctx: StorageHandlerContext): CatalogStorageFormat = withOrigin(ctx) {
    val handler = ctx.STRING().getText
    if (handler.indexOf("HBaseStorageHandler") > 0) {
      val params = Map("storage_handler" -> "org.apache.hadoop.hive.hbase.HBaseStorageHandler",
        "totalSize" -> "0",
        "numRows" -> "0",
        "rawDataSize" -> "0",
      ...
      CatalogStorageFormat.empty.copy(
        //inputFormat = Option("org.apache.hadoop.hive.hbase.HiveHBaseTableInputFormat"),
        //outputFormat = Option("org.apache.hadoop.hive.hbase.HiveHFileOutputFormat"),
        serde = Option("org.apache.hadoop.hive.hbase.HBaseSerDe"),
        serdeProperties = Option(ctx.tablePropertyList).map(visitPropertyKeyValues).getOrElse(Map.empty),
        params = params)
    } else {
      operationNotAllowed(s"STORED BY with storage handler '$handler'", ctx)
    }
  ...
  override protected def withScriptIOSchema(
    ...
    def format(
      ...
      case c: RowFormatSerdeContext =>
        // Use a serde format.
        val CatalogStorageFormat(None, None, None, Some(name), _, props, params) = visitRowFormatSerde(c)
