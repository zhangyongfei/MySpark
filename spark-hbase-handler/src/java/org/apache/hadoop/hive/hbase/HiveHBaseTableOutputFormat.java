...
/**
 * HiveHBaseTableOutputFormat implements HiveOutputFormat for HBase tables. We
 * also need to implement the @deprecated org.apache.hadoop.mapred.OutFormat
...
public class HiveHBaseTableOutputFormat extends TableOutputFormat<ImmutableBytesWritable>
	 ...
	 *            the table info of the corresponding table
	 * @param progress
	...
	@Override
	public org.apache.hadoop.mapred.RecordWriter<ImmutableBytesWritable, Object> getRecordWriter(FileSystem fileSystem,
		...
		public void write(ImmutableBytesWritable key, Object value) throws IOException {
			...
			if (value instanceof Put) {
				...
				throw new IllegalArgumentException(
						"Illegal Argument " + (value == null ? "null" : value.getClass().getName()));
			}
			if (m_walEnabled) {
	...
	static private class MyHiveRecordWriter
	implements RecordWriter {

		private final HTable m_table;
		private final boolean m_walEnabled;

		public MyHiveRecordWriter(HTable table, boolean walEnabled) {
			m_table = table;
			m_walEnabled = walEnabled;
		}
	...
	@Override
	public RecordWriter getHiveRecordWriter(JobConf jobConf, Path finalOutPath, Class<? extends Writable> valueClass,
		...
		final HTable table = new HTable(HBaseConfiguration.create(jobConf), hbaseTableName);
		table.setAutoFlush(false);
		return new MyHiveRecordWriter(table, walEnabled);
	}
}
